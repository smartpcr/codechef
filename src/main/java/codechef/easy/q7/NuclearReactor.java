package codechef.easy.q7;

/* 
 * Created by xd on 8/23/17 10:17 AM
 *
...
public class NuclearReactor {
    public static void main(String args[]) throws NumberFormatException, IOException {
        ...
        int a; // total no. of particles
        int n; // limit on no. of particles in a chamber
        int k; // no. of chambers
        ...
        while (a >= Math.pow(n + 1, k)) {
            a = a - (int) Math.pow(n + 1, k);
        }
        for (int i = k - 1; i >= 0; i--) {
            chambers[i] = a / (int) (Math.pow(n + 1, i));
            a = a - (int) (Math.pow(n + 1, i)) * chambers[i];
        }
